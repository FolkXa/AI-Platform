#!/usr/bin/env python3
"""
Test script for Ollama chat functionality
"""
import asyncio
import io
import pandas as pd
...
async def test_ollama_chat_functionality():
    """Test the Ollama chat functionality with a sample CSV"""
    ...
    try:
        ...
        print(f"Ollama AI Response: {response}")
        
        # Get session messages
        messages = await chat_use_case.get_session_messages(session.session_id)
        print(f"Total messages: {len(messages)}")
        
        print("Ollama chat functionality test completed successfully!")
        
    except Exception as e:
        print(f"Error testing Ollama chat functionality: {e}")
        print("Make sure Ollama is running and accessible at the configured URL")

if __name__ == "__main__":
    asyncio.run(test_ollama_chat_functionality()) 